...
function getFileIcon(fileType, fileName) {
  ...
  if (fileType.startsWith("image/")) {
    return icons_default.get("image");
  }
  if (fileType === "application/pdf") {
    return icons_default.get("file-pdf");
  }
  if (fileType.startsWith("text/") || ["txt", "md", "json", "xml", "html", "css", "js", "ts", "jsx", "tsx"].includes(extension)) {
    return icons_default.get("file-text");
  }
  if (["doc", "docx"].includes(extension)) {
    return icons_default.get("file-word");
  }
  if (["xls", "xlsx"].includes(extension)) {
    return icons_default.get("file-spreadsheet");
  }
  if (["ppt", "pptx"].includes(extension)) {
    return icons_default.get("file-presentation");
  }
  if (["zip", "rar", "7z", "tar", "gz"].includes(extension)) {
    return icons_default.get("file-zip");
  }
  if (["mp3", "wav", "flac", "aac", "ogg"].includes(extension)) {
    return icons_default.get("file-music");
  }
  if (["mp4", "avi", "mov", "wmv", "flv", "webm"].includes(extension)) {
    return icons_default.get("file-video");
  }
  return icons_default.get("file");
}
var File_default = Node.create({
  name: "file",
  group: "block",
  draggable: true,
  addOptions() {
        ...
        props: {
          handlePaste: (view, event) => {
            ...
            Array.from(files).forEach((file) => {
              ...
              const onUploadComplete = (attrs, completedUpload) => {
                ...
                view.dispatch(
                  view.state.tr.replaceWith(view.state.history$.prevRanges[0], view.state.history$.prevRanges[1], schema.nodes.file.create(payload)).setMeta(placeholderPlugin, { remove: { id } })
                );
              };
          ...
          handleDrop: (view, event, _sliced, _moved) => {
            ...
            Array.from(files).forEach((file) => {
              ...
              if (!tr.selection.empty)
                tr.deleteSelection();
              tr.setMeta(placeholderPlugin, { add: { id, pos: coordinates.pos }, file });
              view.dispatch(tr);
              const onUploadComplete = (attrs, completedUpload) => {
                ...
                view.dispatch(
                  view.state.tr.replaceWith(pos, pos, schema.nodes.file.create(payload)).setMeta(placeholderPlugin, { remove: { id } })
                );
              };
              uploadFile(file, onUploadComplete);
            });
